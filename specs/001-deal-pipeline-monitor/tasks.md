...
- [ ] T131 Full end-to-end validation: `make setup && make eval && make demo` from clean clone; fix any breakage. Targets: SC-001 (setup+demo <3 min), SC-006 (≥13/15 eval including enrichment scenario T105).
  - Note: Integration tests updated for 5→1 combined LLM risk call (AllRiskSignals); all 48 tests passing.
...
- [X] TS01 [STRETCH] Fourth intervention type `status_recommendation` — add to `models/interventions.py` discriminated union + new prompt + severity-rubric branch. Cut-off: hour 48.
- [X] TS02 [STRETCH] Scale to 40 deals + 23 scenarios — `generate_deals(count=40)` already parametric (T116); add 8 YAML fixtures. Cut-off: hour 50.
- [X] TS03 [STRETCH] Screen 3 standalone "Reviewer Queue" page — add `web/routes/queue.py` + `templates/queue.html`; reuses `_all_open_list.html`. Cut-off: hour 51.
...
- [X] TS12 [STRETCH] Batch approve Watch items — add "Approve all Watch" button in the All Open tab header of `_all_open_list.html`, rendered only when ≥1 pending watch-severity intervention exists; new route `POST /interventions/batch-approve` with `severity_filter: str = Form('watch')` that loops `approve_intervention_atomic` for each matching pending intervention and returns a refreshed list fragment; `hx-confirm` on the button shows the count of items that will be affected. Scope is strictly Watch-severity: act/escalate items require individual review. Estimated 2h. Cut-off: **hour 53**.
- [ ] TS13 [STRETCH] Read-only "Ask the agent" sidebar — per-deal natural-language Q&A grounded on DB state (deals, events, observations, signals, interventions). **Scope is strictly read-only**: no tool calls that mutate state, no command execution ("send a nudge" is explicitly out of scope for v1). Architecture: single `call_structured()` call — no LangGraph loop — with context bundled from existing DAO reads. New `llm/prompts/ask.py` with `AskAnswer` Pydantic schema (`answer: str`, `citations: list[{deal_id, observation_id?, event_id?}]`, `confidence: float`). New `db/dao.py::build_ask_context(conn, deal_id) -> dict` composing `get_deal` + `get_events` + `get_observations` + `get_interventions` + `fetch_communication_content` (all existing). New `web/routes/ask.py` with `POST /api/deals/{deal_id}/ask` returning HTMX partial; register in `routes/__init__.py`. New `_ask_panel.html` (sidebar, Alpine-driven input + loading) and `_ask_message.html` (bubble with citation chips linking to deal/observation IDs). Mount as `<aside>` on `deal_detail.html` using the sidebar pattern from `brief.html`. Ephemeral/in-page only — no persistence, no new tables; audit trail comes for free via Langfuse traces (call_name=`ask_deal`). Reuse: `call_structured()` (retries, cost, tracing), all DAO read helpers, `ChatPromptTemplate` pattern, card styling from `_macros.html`. Write fresh: prompt + schema, context bundler, route, 2 templates. Smoke test: POST returns 200 and answer cites real IDs from the context. Estimated 4–6h. **Cross-deal queries (v2 extension, +3h)**: add `POST /api/ask` (no deal scope) that bundles portfolio-wide context — `get_live_deals` + `get_latest_observation_per_deal` + open interventions — for questions like "which deals are silent >10 days in ROFR?" or "which deals share an issuer with an open escalate?"; mount panel in `brief.html` sidebar. Context budget becomes the real constraint — truncate to top-N most-recent observations and summarize rather than dumping every deal. Cut-off: **hour 54**.
